const Checkbox = ({ returnType = 'object', size = '2.4rem', value, disabled, className, checkColor, onChange }) => {
    if (returnType === 'string') {
        const dataId = uuidv4()
        $('body').on('change', `.checkbox-container[chcbx-id=${dataId}] > input`, onChange)
        return `<label ${dataId ? `chcbx-id=${dataId}` : ''} class="checkbox-container row ${className}" style="width: ${size}; height: ${size}" >
